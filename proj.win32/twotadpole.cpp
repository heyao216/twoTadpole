    ...
    int       nCmdShow)
{
    ...
    return TwotadpolePlayer::createAndRun();
}

int TwotadpolePlayer::createAndRun(void)
{
    TwotadpolePlayer *host = TwotadpolePlayer::sharedInstance();
    int ret = host->run();
    TwotadpolePlayer::purgeSharedInstance();
    return ret;
}

TwotadpolePlayer *TwotadpolePlayer::s_sharedInstance = NULL;

TwotadpolePlayer *TwotadpolePlayer::sharedInstance(void)
{
    if (!s_sharedInstance)
    {
        s_sharedInstance = new TwotadpolePlayer();
    }
    return s_sharedInstance;
}

void TwotadpolePlayer::purgeSharedInstance(void)
{
    if (s_sharedInstance)
    {
...
TwotadpolePlayer::TwotadpolePlayer(void)
: m_app(NULL)
, m_hwnd(NULL)
, m_exit(TRUE)
...
int TwotadpolePlayer::run(void)
{
    ...
    do
    {
        ...
        SetWindowTextA(m_hwnd, "twotadpole-player");
...
void TwotadpolePlayer::loadProjectConfig(void)
{
    vector<string> args;
...
void TwotadpolePlayer::createViewMenu(void)
{
...
void TwotadpolePlayer::updateMenu(void)
{
    HMENU menu = GetMenu(m_hwnd);
    HMENU viewMenu = GetSubMenu(menu, 1);
...
void TwotadpolePlayer::updateWindowTitle(void)
{
}

void TwotadpolePlayer::relaunch(void)
{
...
void TwotadpolePlayer::writeDebugLog(const char *log)
{
...
// menu callback
void TwotadpolePlayer::onFileOpenProject(void)
{
...
void TwotadpolePlayer::onFileCreateProjectShortcut(void)
{
    ...
    if (SUCCEEDED(hres))
    {
        ...
        psl->SetDescription(L"TwotadpolePlayer");
...
void TwotadpolePlayer::onFileProjectConfig(void)
{
...
void TwotadpolePlayer::onFileRelaunch(void)
{
    relaunch();
}

void TwotadpolePlayer::onFileExit(void)
{
    DestroyWindow(m_hwnd);
}

void TwotadpolePlayer::onViewChangeFrameSize(int viewMenuID)
{
...
void TwotadpolePlayer::onViewChangeOrientation(int viewMenuID)
{
    bool isLandscape = m_project.isLandscapeFrame();
...
void TwotadpolePlayer::onViewChangeZoom(int scaleMode)
{
    float scale = 1.0f;
...
void TwotadpolePlayer::onHelpAbout(void)
{
    DialogBox(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_ABOUTBOX), m_hwnd, AboutDialogCallback);
}

// windows callback
LRESULT TwotadpolePlayer::WindowProc(UINT message, WPARAM wParam, LPARAM lParam, BOOL* pProcessed)
{
    int wmId, wmEvent;
    TwotadpolePlayer *host = TwotadpolePlayer::sharedInstance();
...
INT_PTR CALLBACK TwotadpolePlayer::AboutDialogCallback(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
